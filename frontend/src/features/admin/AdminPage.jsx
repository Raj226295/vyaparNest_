...
const defaultProviderProfileDesign = {
  template: 'Modern',
  primaryColor: '#5d31e1',
  accentColor: '#f2a915',
  ...
  textColor: '#18213a',
  businessName: '',
  tagline: 'Trusted service provider for growing businesses',
  bio: 'We deliver reliable, high-quality services with clear communication and customer-focused support.',
  coverImage: '/market-sections/guide-growth.png',
  avatarImage: '',
...
const initialAdminServiceRequests = [
  {id:'REQ-9012',customer:'Rohan Gupta',email:'rohan.gupta@email.com',service:'GST Filing',category:'Finance & Legal',provider:'Unassigned',status:'Pending',priority:'High',budget:'₹4,500',requested:'May 31, 2025',time:'10:30 AM',initials:'RG'},
  {id:'REQ-9011',customer:'Mansi Sharma',email:'mansi.sharma@email.com',service:'Business Website',category:'Web Development',provider:'Rohit Sharma',status:'Accepted',priority:'Medium',budget:'₹18,000',requested:'May 30, 2025',time:'04:15 PM',initials:'MS'},
  {id:'REQ-9010',customer:'Kavya Verma',email:'kavya.verma@email.com',service:'SEO Audit',category:'Digital Marketing',provider:'Priya Patel',status:'In Progress',priority:'High',budget:'₹9,500',requested:'May 30, 2025',time:'11:20 AM',initials:'KV'},
  {id:'REQ-9009',customer:'Aman Jha',email:'aman.jha@email.com',service:'Company Registration',category:'Legal Services',provider:'Amit Verma',status:'Completed',priority:'Low',budget:'₹6,000',requested:'May 29, 2025',time:'09:45 AM',initials:'AJ'},
  {id:'REQ-9008',customer:'Nisha Roy',email:'nisha.roy@email.com',service:'Home Deep Cleaning',category:'Home Services',provider:'Neha Gupta',status:'Rejected',priority:'Medium',budget:'₹2,000',requested:'May 28, 2025',time:'02:10 PM',initials:'NR'},
  {id:'REQ-9007',customer:'Vikram Singh',email:'vikram.singh@email.com',service:'Mobile App Development',category:'App Development',provider:'Unassigned',status:'Pending',priority:'High',budget:'₹25,000',requested:'May 28, 2025',time:'10:05 AM',initials:'VS'},
  {id:'REQ-9006',customer:'Sneha Reddy',email:'sneha.reddy@email.com',service:'Content Writing',category:'Writing Services',provider:'Pooja Mehta',status:'In Progress',priority:'Low',budget:'₹1,800',requested:'May 27, 2025',time:'03:30 PM',initials:'SR'},
  {id:'REQ-9005',customer:'Arjun Nair',email:'arjun.nair@email.com',service:'Video Editing',category:'Multimedia',provider:'Karan Malhotra',status:'Completed',priority:'Medium',budget:'₹3,200',requested:'May 27, 2025',time:'12:00 PM',initials:'AN'},
]
const initialAdminPayments = [
  {id:'PAY-1001',provider:'TaxExpert India',email:'contact@taxexpert.com',initials:'TE',plan:'Premium Plan',cycle:'Monthly',amount:'₹12,000',method:'Razorpay',status:'Paid',date:'Jul 24, 2026',time:'10:30 AM',renewal:'Aug 24, 2026'},
  {id:'PAY-1002',provider:'Smart Website Development',email:'info@smartwebdev.com',initials:'SW',plan:'Standard Plan',cycle:'Monthly',amount:'₹6,000',method:'Stripe',status:'Paid',date:'Jul 24, 2026',time:'09:15 AM',renewal:'Aug 24, 2026'},
  {id:'PAY-1003',provider:'RankSprint Media',email:'hello@ranksprint.com',initials:'RS',plan:'Basic Plan',cycle:'Monthly',amount:'₹3,000',method:'PayPal',status:'Paid',date:'Jul 23, 2026',time:'04:20 PM',renewal:'Aug 23, 2026'},
  {id:'PAY-1004',provider:'AuditBridge Partners',email:'contact@auditbridge.com',initials:'AB',plan:'Premium Plan',cycle:'Yearly',amount:'₹16,000',method:'Razorpay',status:'Paid',date:'Jul 22, 2026',time:'11:45 AM',renewal:'Jul 22, 2027'},
  {id:'PAY-1005',provider:'HomeFix Experts',email:'support@homefix.com',initials:'HF',plan:'Standard Plan',cycle:'Monthly',amount:'₹6,000',method:'Stripe',status:'Pending',date:'Jul 21, 2026',time:'02:10 PM',renewal:'Aug 21, 2026'},
  {id:'PAY-1006',provider:'Digital Marketing Pro',email:'hello@dmpro.com',initials:'DM',plan:'Basic Plan',cycle:'Monthly',amount:'₹3,000',method:'Razorpay',status:'Paid',date:'Jul 20, 2026',time:'06:05 PM',renewal:'Aug 20, 2026'},
  {id:'PAY-1007',provider:'Legal Solutions',email:'info@legalsolutions.com',initials:'LS',plan:'Premium Plan',cycle:'Monthly',amount:'₹12,000',method:'Stripe',status:'Paid',date:'Jul 19, 2026',time:'01:25 PM',renewal:'Aug 19, 2026'},
  {id:'PAY-1008',provider:'Web Creators',email:'contact@webcreators.com',initials:'WC',plan:'Basic Plan',cycle:'Monthly',amount:'₹3,000',method:'PayPal',status:'Refunded',date:'Jul 18, 2026',time:'10:00 AM',renewal:'—'},
]
const rowsPerPage = 5
...
const managementSectionConfigs = {
  ...
  portfolio: {
    ...
    description: 'Design and control every service provider’s public profile from one place.',
    primaryAction: 'Manage Provider Profile',
    rows: portfolioRows,
    columns: [
...
function AdminIcon({ type, className = '' }) {
  ...
  switch (type) {
    ...
    case 'eye':
      return <svg viewBox="0 0 24 24" className={classes} fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 12s3.2-5.5 9-5.5 9 5.5 9 5.5-3.2 5.5-9 5.5S3 12 3 12Z"/><circle cx="12" cy="12" r="2.5"/></svg>
    case 'trash':
      return <svg viewBox="0 0 24 24" className={classes} fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 7h16M9 7V4h6v3M6.5 7l1 13h9l1-13M10 11v5M14 11v5"/></svg>
    case 'filter':
      return <svg viewBox="0 0 24 24" className={classes} fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 5h16l-6.2 7v5l-3.6 2v-7L4 5Z"/></svg>
    case 'download':
      return <svg viewBox="0 0 24 24" className={classes} fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 4v11m-4-4 4 4 4-4M4 18v2h16v-2"/></svg>
    case 'calendar':
      return <svg viewBox="0 0 24 24" className={classes} fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M8 3v4M16 3v4M4 9h16"/></svg>
    case 'sun':
      return (
        <svg viewBox="0 0 24 24" className={classes} fill="none" stroke="currentColor" strokeWidth="1.8">
...
function AdminPage() {
  ...
  const [profileProviderId, setProfileProviderId] = useState(providerRows[0]?.id || '')
  const [providerProfileDesigns, setProviderProfileDesigns] = useState(() => {
    if (typeof window === 'undefined') return {}
    try {
      return JSON.parse(window.localStorage.getItem(providerProfileDesignsStorageKey) || '{}')
    } catch {
      return {}
    }
  })
  const [serviceRequestsData, setServiceRequestsData] = useState(initialAdminServiceRequests)
  ...
  const [projectsData, setProjectsData] = useState(() => projectRows.map((project,index)=>({...project,category:['Finance & Legal','Web Development','Digital Marketing','Legal Services','Home Services'][index],email:['rohan.gupta@email.com','mansi.sharma@email.com','kavya.verma@email.com','aman.jha@email.com','nisha.roy@email.com'][index],providerSite:['taxexpert.com','smartwebdev.com','ranksprint.com','auditbridge.com','homefix.com'][index],days:['12 days left','18 days left','14 days left','Completed','Cancelled'][index],initials:['RG','MS','KV','AJ','NR'][index],providerInitials:['TE','SW','RM','AB','HF'][index]})))
  const [projectStatusFilter, setProjectStatusFilter] = useState('All')
  const [projectServiceFilter, setProjectServiceFilter] = useState('All')
  const [projectProviderFilter, setProjectProviderFilter] = useState('All')
  const [, setProjectPage] = useState(1)
  const [projectDialog, setProjectDialog] = useState(null)
  const [paymentsData] = useState(initialAdminPayments)
  ...
  const handleRowAction = (sectionId, actionLabel, row) => {
    ...
    const nextAction = () => {
      if (actionLabel === 'Manage Profile') {
        openAdminSection('portfolio')
        setProfileProviderId(row.id)
        pushToast(`Opened ${cleanText(row.name)} profile designer.`)
        return
      }
                      ...
                      <div className="vn-admin-row-actions">
                        {getRowActions(activeSection, row).map((action) => (
                          <button
                            ...
                            type="button"
                            className={action.label.startsWith('View') ? 'vn-admin-view-icon' : /delete|reject|block|refund/i.test(action.label) ? 'is-danger' : ''}
                            aria-label={action.label.startsWith('View') ? `${action.label} ${cleanText(row.id)}` : undefined}
                            title={action.label.startsWith('View') ? action.label : undefined}
                            onClick={() => handleRowAction(activeSection, action.label, row)}
                          >
                            {action.label.startsWith('View') ? <AdminIcon type="eye"/> : action.label}
                          </button>
                        ))}
                      </div>
                    </td>
  ...
  const renderPaymentsSection = () => {
    const query=searchQuery.trim().toLowerCase();const filtered=paymentsData.filter((payment)=>(paymentPlanFilter==='All'||payment.plan===paymentPlanFilter)&&(paymentStatusFilter==='All'||payment.status===paymentStatusFilter)&&(paymentMethodFilter==='All'||payment.method===paymentMethodFilter)&&(!query||[payment.id,payment.provider,payment.email].some((value)=>value.toLowerCase().includes(query))))
    const exportPayments=()=>{const csv=[['Payment ID','Provider','Email','Plan','Amount','Method','Status','Payment Date','Next Renewal'],...filtered.map((item)=>[item.id,item.provider,item.email,item.plan,item.amount,item.method,item.status,item.date,item.renewal])].map((row)=>row.map((cell)=>`"${cell}"`).join(',')).join('\n');const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));link.download='subscription-payments.csv';link.click();URL.revokeObjectURL(link.href);pushToast(`${filtered.length} payments exported.`)}
    const stats=[['wallet','Total Subscription Payments','₹4,85,600','All time','violet'],['calendar','This Month','₹82,450','↑ 14.6% from last month','green'],['check','Successful Payments','152','All time','blue'],['history','Pending Payments','8','Pending','orange'],['wallet','Refunded Payments','₹12,900','All time','pink']]
    return <div className="vn-payments-page"><header className="vn-payments-title"><div><h2>Payments</h2><p>All payments received from service providers for their subscription plans.</p></div><div><button onClick={exportPayments}><AdminIcon type="download"/>Export</button><button className="is-primary" onClick={()=>pushToast('Subscription report generated successfully.')}><AdminIcon type="document"/>Subscription Report</button></div></header>
      <section className="vn-payment-stat-grid">{stats.map(([icon,label,value,note,tone])=><article key={label}><i className={`is-${tone}`}><AdminIcon type={icon}/></i><div><span>{label}</span><strong>{value}</strong><small className={tone==='green'?'is-up':''}>{note}</small></div></article>)}</section>
      <section className="vn-payment-card"><div className="vn-payment-filters"><label><AdminIcon type="search"/><input value={searchQuery} onChange={(event)=>setSearchQuery(event.target.value)} placeholder="Search by provider name or email..."/></label><select value={paymentPlanFilter} onChange={(event)=>setPaymentPlanFilter(event.target.value)}><option value="All">All Plans</option><option>Basic Plan</option><option>Standard Plan</option><option>Premium Plan</option></select><select value={paymentStatusFilter} onChange={(event)=>setPaymentStatusFilter(event.target.value)}><option value="All">All Status</option><option>Paid</option><option>Pending</option><option>Refunded</option></select><select value={paymentMethodFilter} onChange={(event)=>setPaymentMethodFilter(event.target.value)}><option value="All">All Payment Methods</option><option>Razorpay</option><option>Stripe</option><option>PayPal</option></select><label className="is-date"><AdminIcon type="calendar"/><input type="date" aria-label="Select payment date range"/></label><button onClick={()=>pushToast(`${filtered.length} matching payments found.`)}><AdminIcon type="filter"/>Filter</button></div>
      <div className="vn-payment-table-wrap"><table className="vn-payment-table"><thead><tr><th><input type="checkbox" aria-label="Select all payments"/></th><th>Payment ID</th><th>Service Provider</th><th>Plan</th><th>Amount</th><th>Payment Method</th><th>Status</th><th>Payment Date</th><th>Next Renewal</th><th>Actions</th></tr></thead><tbody>{filtered.map((payment,index)=><tr key={payment.id}><td><input type="checkbox" aria-label={`Select ${payment.id}`}/></td><td><strong>{payment.id}</strong></td><td><div className="vn-payment-provider"><i className={`is-${index%5}`}>{payment.initials}</i><span><strong>{payment.provider}</strong><small>{payment.email}</small></span></div></td><td><div className={`vn-payment-plan is-${payment.plan.split(' ')[0].toLowerCase()}`}><i><AdminIcon type="shield"/></i><span><strong>{payment.plan}</strong><small>{payment.cycle}</small></span></div></td><td><strong>{payment.amount}</strong></td><td><div className={`vn-payment-method is-${payment.method.toLowerCase()}`}><i>{payment.method==='Stripe'?'S':payment.method==='PayPal'?'P':'➤'}</i><strong>{payment.method}</strong></div></td><td><StatusBadge value={payment.status}/></td><td><strong>{payment.date}</strong><small>{payment.time}</small></td><td><strong>{payment.renewal}</strong></td><td><button className="vn-payment-view" onClick={()=>setPaymentDialog(payment)} aria-label={`View ${payment.id}`}><AdminIcon type="eye"/></button></td></tr>)}</tbody></table></div><footer className="vn-payment-pagination"><span>Showing 1 to {filtered.length} of 152 payments</span><div><button disabled>‹</button><button className="is-active">1</button><button>2</button><button>3</button><button>…</button><button>19</button><button>›</button></div><select><option>10 / page</option><option>25 / page</option></select></footer></section>
      {paymentDialog?<div className="vn-payment-dialog-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&setPaymentDialog(null)}><div className="vn-payment-dialog"><header><div><h3>Payment Details</h3><p>{paymentDialog.id} · {paymentDialog.provider}</p></div><button onClick={()=>setPaymentDialog(null)}>×</button></header><div className="vn-payment-detail"><i><AdminIcon type="wallet"/></i><h4>{paymentDialog.amount}</h4><StatusBadge value={paymentDialog.status}/><dl><dt>Provider</dt><dd>{paymentDialog.provider}</dd><dt>Plan</dt><dd>{paymentDialog.plan} ({paymentDialog.cycle})</dd><dt>Method</dt><dd>{paymentDialog.method}</dd><dt>Payment Date</dt><dd>{paymentDialog.date}</dd><dt>Next Renewal</dt><dd>{paymentDialog.renewal}</dd><dt>Email</dt><dd>{paymentDialog.email}</dd></dl><button onClick={()=>setPaymentDialog(null)}>Done</button></div></div></div>:null}
    </div>
  }

  const renderProjectsSection = () => {
    const query=searchQuery.trim().toLowerCase()
    const filtered=projectsData.filter((project)=>(projectStatusFilter==='All'||project.status===projectStatusFilter)&&(projectServiceFilter==='All'||project.category===projectServiceFilter)&&(projectProviderFilter==='All'||project.provider===projectProviderFilter)&&(!query||[project.id,project.title,project.customer,project.provider].some((value)=>value.toLowerCase().includes(query))))
    const services=[...new Set(projectsData.map((item)=>item.category))];const providers=[...new Set(projectsData.map((item)=>item.provider))]
    const exportProjects=()=>{const csv=[['Project ID','Project','Customer','Provider','Status','Progress','Deadline','Value'],...filtered.map((item)=>[item.id,item.title,item.customer,item.provider,item.status,item.progress,item.deadline,item.value])].map((row)=>row.map((cell)=>`"${cell}"`).join(',')).join('\n');const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));link.download='orders-projects.csv';link.click();URL.revokeObjectURL(link.href);pushToast(`${filtered.length} projects exported.`)}
    const updateProject=(project,status)=>{setProjectsData((current)=>current.map((item)=>item.id===project.id?{...item,status,progress:status==='Completed'?100:item.progress,days:status}:item));setProjectDialog(null);pushToast(`${project.id} marked ${status}.`,status==='Cancelled'?'warning':'success')}
    const stats=[['briefcase','Total Projects','124','All time','violet'],['history','Active Projects','68','54.8% of total','orange'],['check','Completed Projects','38','30.6% of total','blue'],['calendar','Pending Projects','12','9.7% of total','amber'],['close','Cancelled Projects','6','4.8% of total','red']]
    return <div className="vn-projects-page"><header className="vn-projects-title"><div><h2>Orders / Projects</h2><p>Track and manage all customer orders and projects across the platform.</p></div><div><button onClick={exportProjects}><AdminIcon type="download"/>Export</button><button className="is-primary" onClick={()=>{setProjectStatusFilter('Active');setProjectPage(1);pushToast('Active projects loaded.')}}><AdminIcon type="briefcase"/>Open Active Projects</button></div></header>
      <section className="vn-project-stat-grid">{stats.map(([icon,label,value,note,tone])=><article key={label}><i className={`is-${tone}`}><AdminIcon type={icon}/></i><div><span>{label}</span><strong>{value}</strong><small className={tone==='red'?'is-down':''}>{note}</small></div></article>)}</section>
      <section className="vn-project-card"><div className="vn-project-filters"><label><AdminIcon type="search"/><input value={searchQuery} onChange={(event)=>{setSearchQuery(event.target.value);setProjectPage(1)}} placeholder="Search orders / projects..."/></label><select value={projectStatusFilter} onChange={(event)=>{setProjectStatusFilter(event.target.value);setProjectPage(1)}}><option value="All">All Status</option><option>Active</option><option>Pending</option><option>Completed</option><option>Cancelled</option></select><select value={projectServiceFilter} onChange={(event)=>{setProjectServiceFilter(event.target.value);setProjectPage(1)}}><option value="All">All Services</option>{services.map((item)=><option key={item}>{item}</option>)}</select><select value={projectProviderFilter} onChange={(event)=>{setProjectProviderFilter(event.target.value);setProjectPage(1)}}><option value="All">All Providers</option>{providers.map((item)=><option key={item}>{item}</option>)}</select><label className="is-date"><AdminIcon type="calendar"/><input type="date" aria-label="Select date range" onChange={()=>setProjectPage(1)}/></label><button onClick={()=>pushToast(`${filtered.length} matching projects found.`)}><AdminIcon type="filter"/>Filter</button></div>
      <div className="vn-project-table-wrap"><table className="vn-project-table"><thead><tr><th><input type="checkbox" aria-label="Select all projects"/></th><th>Project ID</th><th>Project</th><th>Customer</th><th>Provider</th><th>Status</th><th>Progress</th><th>Deadline</th><th>Value</th><th>Actions</th></tr></thead><tbody>{filtered.map((project,index)=><tr key={project.id}><td><input type="checkbox" aria-label={`Select ${project.id}`}/></td><td><strong className="vn-project-id">{project.id}</strong></td><td><strong>{project.title}</strong><small>{project.category}</small></td><td><div className="vn-project-person"><i className={`is-${index%3}`}>{project.initials}</i><span><strong>{project.customer}</strong><small>{project.email}</small></span></div></td><td><div className="vn-project-person"><i className={`is-provider is-${index%4}`}>{project.providerInitials}</i><span><strong>{project.provider}</strong><small>{project.providerSite}</small></span></div></td><td><StatusBadge value={project.status}/></td><td><div className={`vn-project-progress is-${project.status.toLowerCase()}`}><strong>{project.progress}%</strong><i><span style={{width:`${project.progress}%`}}/></i></div></td><td><strong>{project.deadline}</strong><small className={`is-${project.status.toLowerCase()}`}>{project.days}</small></td><td><strong>{project.value}</strong></td><td><div className="vn-project-actions"><button onClick={()=>setProjectDialog({type:'view',project})} aria-label={`View ${project.id}`}><AdminIcon type="eye"/></button><button onClick={()=>setProjectDialog({type:'actions',project})} aria-label={`More actions for ${project.id}`}>⋮</button></div></td></tr>)}</tbody></table></div>
      <footer className="vn-project-pagination"><span>Showing 1 to {filtered.length} of 124 projects</span><div><button disabled>‹</button><button className="is-active">1</button><button>2</button><button>3</button><button>…</button><button>25</button><button>›</button></div><select><option>10 / page</option><option>25 / page</option></select></footer></section>
      <section className="vn-project-value-grid">{[['wallet','Total Value','₹97,000','All projects','violet'],['history','Outstanding Value','₹60,500','Active + Pending','orange'],['check','Completed Value','₹16,000','From completed projects','green'],['chart','Average Project Value','₹17,741','Across all projects','violet']].map(([icon,label,value,note,tone])=><article key={label}><i className={`is-${tone}`}><AdminIcon type={icon}/></i><div><span>{label}</span><strong>{value}</strong><small>{note}</small></div></article>)}</section>
      {projectDialog?<div className="vn-project-dialog-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&setProjectDialog(null)}><div className="vn-project-dialog"><header><div><h3>{projectDialog.type==='view'?'Project Details':'Project Actions'}</h3><p>{projectDialog.project.id} · {projectDialog.project.title}</p></div><button onClick={()=>setProjectDialog(null)}>×</button></header>{projectDialog.type==='view'?<div className="vn-project-detail"><i><AdminIcon type="briefcase"/></i><h4>{projectDialog.project.title}</h4><StatusBadge value={projectDialog.project.status}/><dl><dt>Customer</dt><dd>{projectDialog.project.customer}</dd><dt>Provider</dt><dd>{projectDialog.project.provider}</dd><dt>Progress</dt><dd>{projectDialog.project.progress}%</dd><dt>Deadline</dt><dd>{projectDialog.project.deadline}</dd><dt>Project Value</dt><dd>{projectDialog.project.value}</dd></dl><button onClick={()=>setProjectDialog(null)}>Done</button></div>:<div className="vn-project-action-menu"><button onClick={()=>setProjectDialog({type:'view',project:projectDialog.project})}><AdminIcon type="eye"/>View Project</button><button onClick={()=>updateProject(projectDialog.project,'Active')}><AdminIcon type="history"/>Mark Active</button><button onClick={()=>updateProject(projectDialog.project,'Completed')}><AdminIcon type="check"/>Mark Completed</button><button className="is-danger" onClick={()=>updateProject(projectDialog.project,'Cancelled')}><AdminIcon type="close"/>Cancel Project</button></div>}</div></div>:null}
    </div>
  }

  const renderServiceRequestsSection = () => {
    const query = searchQuery.trim().toLowerCase()
    const filtered = serviceRequestsData.filter((item)=>(requestStatusFilter==='All'||item.status===requestStatusFilter)&&(requestServiceFilter==='All'||item.service===requestServiceFilter)&&(requestPriorityFilter==='All'||item.priority===requestPriorityFilter)&&(!query||[item.id,item.customer,item.email,item.service,item.provider].some((value)=>value.toLowerCase().includes(query))))
    const pageSize = 8
    const totalPages = Math.max(1,Math.ceil(filtered.length/pageSize))
    const page = Math.min(requestPage,totalPages)
    const visible = filtered.slice((page-1)*pageSize,page*pageSize)
    const allSelected = visible.length>0&&visible.every((item)=>selectedRows.includes(item.id))
    const services = [...new Set(serviceRequestsData.map((item)=>item.service))]
    const exportRequests = () => {const csv=[['Request ID','Customer','Email','Service','Provider','Status','Priority','Budget'],...filtered.map((item)=>[item.id,item.customer,item.email,item.service,item.provider,item.status,item.priority,item.budget])].map((row)=>row.map((cell)=>`"${String(cell).replaceAll('"','""')}"`).join(',')).join('\n');const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));link.download='service-requests.csv';link.click();URL.revokeObjectURL(link.href);pushToast(`${filtered.length} service requests exported.`)}
    const assignProvider = (request,providerName) => {setServiceRequestsData((current)=>current.map((item)=>item.id===request.id?{...item,provider:providerName,status:item.status==='Pending'?'Accepted':item.status}:item));setRequestDialog(null);pushToast(`${providerName} assigned to ${request.id}.`)}
    const removeRequest = (request) => {setServiceRequestsData((current)=>current.filter((item)=>item.id!==request.id));setRequestDialog(null);pushToast(`${request.id} deleted.`,'warning')}
    const stats=[['users','Total Requests','238','↑ 12.5% this month','violet'],['clipboard','Pending','58','↑ 8.2% this month','orange'],['check','Accepted','96','↑ 14.7% this month','green'],['history','In Progress','42','↑ 5.1% this month','blue'],['check','Completed','34','↑ 3.6% this month','pink'],['warning','Rejected','8','↓ 2.5% this month','red']]
    return <div className="vn-requests-page is-read-only"><header className="vn-requests-title"><div><h2>Service Requests</h2><p>View incoming customer requests and inspect complete request details in read-only mode.</p></div><div><span className="vn-request-readonly-badge"><AdminIcon type="eye"/>View Only</span><button onClick={exportRequests}><AdminIcon type="download"/>Export</button></div></header>
      <section className="vn-request-stat-grid">{stats.map(([icon,label,value,delta,tone])=><article key={label}><i className={`is-${tone}`}><AdminIcon type={icon}/></i><div><span>{label}</span><strong>{value}</strong><small className={tone==='red'?'is-down':''}>{delta}</small></div></article>)}</section>
      <section className="vn-request-layout"><main className="vn-request-card"><div className="vn-request-filters"><label><AdminIcon type="search"/><input value={searchQuery} onChange={(event)=>{setSearchQuery(event.target.value);setRequestPage(1)}} placeholder="Search service requests..."/></label><select value={requestStatusFilter} onChange={(event)=>{setRequestStatusFilter(event.target.value);setRequestPage(1)}}><option value="All">All Status</option><option>Pending</option><option>Accepted</option><option>In Progress</option><option>Completed</option><option>Rejected</option></select><select value={requestServiceFilter} onChange={(event)=>{setRequestServiceFilter(event.target.value);setRequestPage(1)}}><option value="All">All Services</option>{services.map((service)=><option key={service}>{service}</option>)}</select><select value={requestPriorityFilter} onChange={(event)=>{setRequestPriorityFilter(event.target.value);setRequestPage(1)}}><option value="All">All Priorities</option><option>High</option><option>Medium</option><option>Low</option></select><button onClick={()=>pushToast(`${filtered.length} matching requests found.`)}><AdminIcon type="filter"/>Filter</button></div>
      <div className="vn-request-table-wrap"><table className="vn-request-table"><thead><tr><th><input type="checkbox" checked={allSelected} onChange={()=>setSelectedRows((current)=>allSelected?current.filter((id)=>!visible.some((item)=>item.id===id)):[...new Set([...current,...visible.map((item)=>item.id)])])}/></th><th>Request ID</th><th>Customer</th><th>Service</th><th>Assigned Provider</th><th>Status</th><th>Priority</th><th>Budget</th><th>Requested On</th><th>Actions</th></tr></thead><tbody>{visible.map((request,index)=><tr key={request.id}><td><input type="checkbox" checked={selectedRows.includes(request.id)} onChange={()=>setSelectedRows((current)=>current.includes(request.id)?current.filter((id)=>id!==request.id):[...current,request.id])}/></td><td><strong>{request.id}</strong></td><td><div className="vn-request-person"><i className={`is-${['violet','sand','blue','green'][index%4]}`}>{request.initials}</i><span><strong>{request.customer}</strong><small>{request.email}</small></span></div></td><td><strong>{request.service}</strong><small>{request.category}</small></td><td><div className="vn-request-provider"><i><AdminIcon type={request.provider==='Unassigned'?'history':'profile'}/></i><strong>{request.provider}</strong></div></td><td><StatusBadge value={request.status}/></td><td><span className={`vn-request-priority is-${request.priority.toLowerCase()}`}><i/>{request.priority}</span></td><td><strong>{request.budget}</strong></td><td><strong>{request.requested}</strong><small>{request.time}</small></td><td><div className="vn-request-actions"><button onClick={()=>setRequestDialog({type:'view',request})} aria-label={`View ${request.id}`}><AdminIcon type="eye"/></button><button onClick={()=>setRequestDialog({type:'assign',request})} aria-label={`Assign ${request.id}`}><AdminIcon type="profile"/></button><button className="is-danger" onClick={()=>setRequestDialog({type:'delete',request})} aria-label={`Delete ${request.id}`}><AdminIcon type="trash"/></button></div></td></tr>)}</tbody></table></div>
      <footer className="vn-request-pagination"><span>Showing {filtered.length?((page-1)*pageSize)+1:0} to {Math.min(page*pageSize,filtered.length)} of {filtered.length} results</span><div><button disabled={page===1} onClick={()=>setRequestPage((value)=>Math.max(1,value-1))}>‹</button>{Array.from({length:totalPages},(_,index)=>index+1).map((number)=><button className={page===number?'is-active':''} onClick={()=>setRequestPage(number)} key={number}>{number}</button>)}<button disabled={page===totalPages} onClick={()=>setRequestPage((value)=>Math.min(totalPages,value+1))}>›</button></div><select><option>8 / page</option></select></footer></main>
      <aside className="vn-request-side"><section><h3>Request Overview</h3><div className="vn-request-donut"><span><strong>238</strong>Total</span></div><ul>{[['Pending','58','24.4%','orange'],['Accepted','96','40.3%','green'],['In Progress','42','17.6%','blue'],['Completed','34','14.3%','mint'],['Rejected','8','3.4%','red']].map(([label,count,percent,tone])=><li key={label}><span><i className={`is-${tone}`}/>{label}</span><strong>{count} <small>({percent})</small></strong></li>)}</ul></section><section><h3>Priority Breakdown</h3>{[['High','92','38.7%'],['Medium','88','37.0%'],['Low','58','24.3%']].map(([label,count,percent])=><div className={`vn-request-priority-bar is-${label.toLowerCase()}`} key={label}><span><strong>{label}</strong><b>{count} ({percent})</b></span><i><em style={{width:percent}}/></i></div>)}</section><section><h3>Quick Actions</h3><div className="vn-request-quick">{[['clipboard','All Requests','238',()=>{setRequestStatusFilter('All');setSearchQuery('')}],['profile','My Assignments','42',()=>setRequestStatusFilter('In Progress')],['history','Overdue Requests','12',()=>{setRequestStatusFilter('Pending');pushToast('Overdue pending requests loaded.')}],['users','Unassigned Requests','26',()=>setSearchQuery('Unassigned')]].map(([icon,label,count,action])=><button onClick={action} key={label}><i><AdminIcon type={icon}/></i><span>{label}</span><strong>{count}</strong></button>)}</div></section></aside></section>
      {requestDialog?<div className="vn-request-dialog-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&setRequestDialog(null)}><div className="vn-request-dialog"><header><div><h3>{requestDialog.type==='view'?'Request Details':requestDialog.type==='assign'?'Assign Service Provider':'Delete Request'}</h3><p>{requestDialog.request.id} · {requestDialog.request.customer}</p></div><button onClick={()=>setRequestDialog(null)}>×</button></header>{requestDialog.type==='view'?<div className="vn-request-detail"><i><AdminIcon type="clipboard"/></i><h4>{requestDialog.request.service}</h4><StatusBadge value={requestDialog.request.status}/><dl><dt>Customer</dt><dd>{requestDialog.request.customer}</dd><dt>Email</dt><dd>{requestDialog.request.email}</dd><dt>Provider</dt><dd>{requestDialog.request.provider}</dd><dt>Priority</dt><dd>{requestDialog.request.priority}</dd><dt>Budget</dt><dd>{requestDialog.request.budget}</dd><dt>Requested</dt><dd>{requestDialog.request.requested}</dd></dl><button onClick={()=>setRequestDialog(null)}>Done</button></div>:null}{requestDialog.type==='assign'?<form onSubmit={(event)=>{event.preventDefault();assignProvider(requestDialog.request,new FormData(event.currentTarget).get('provider'))}}><label>Choose Provider<select name="provider" defaultValue={requestDialog.request.provider==='Unassigned'?'':requestDialog.request.provider} required><option value="" disabled>Select a provider</option>{providerRows.filter((item)=>item.status==='Active').map((item)=><option key={item.id}>{item.name}</option>)}</select></label><label>Assignment Note<textarea name="note" rows="3" placeholder="Optional note for the provider"/></label><footer><button type="button" onClick={()=>setRequestDialog(null)}>Cancel</button><button className="is-primary">Assign Provider</button></footer></form>:null}{requestDialog.type==='delete'?<div className="vn-request-delete"><i><AdminIcon type="warning"/></i><p>Delete <strong>{requestDialog.request.id}</strong>? This request will be removed from the table.</p><footer><button onClick={()=>setRequestDialog(null)}>Cancel</button><button className="is-danger" onClick={()=>removeRequest(requestDialog.request)}>Delete Request</button></footer></div>:null}</div></div>:null}
    </div>
  }

  const renderProfileManagementSection = () => {
    const provider = providerRows.find((item) => item.id === profileProviderId) || providerRows[0]
    const design = { ...defaultProviderProfileDesign, businessName: provider?.name || '', ...(providerProfileDesigns[provider?.id] || {}) }
    const updateDesign = (changes) => setProviderProfileDesigns((current) => ({ ...current, [provider.id]: { ...design, ...changes } }))
    const saveDesign = () => {
      const nextDesigns = { ...providerProfileDesigns, [provider.id]: { ...design, providerName: provider.name } }
      setProviderProfileDesigns(nextDesigns)
      window.localStorage.setItem(providerProfileDesignsStorageKey, JSON.stringify(nextDesigns))
      pushToast(`${design.businessName || provider.name} profile design saved and published.`)
    }
    const resetDesign = () => {
      setProviderProfileDesigns((current) => ({ ...current, [provider.id]: { ...defaultProviderProfileDesign, businessName: provider.name } }))
      pushToast(`${provider.name} profile design reset.`, 'warning')
    }
    const initials = (design.businessName || provider.name).split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()
    const providerServices = servicesData.filter((service) => service.provider === provider.name).slice(0, 3)
    const providerPortfolio = portfolioRows.filter((item) => item.provider === provider.name).slice(0, 3)
    return <div className="vn-profile-manager">
      <header className="vn-profile-manager-title"><div><span><AdminIcon type="gallery"/></span><div><h2>Provider Profile Management</h2><p>Customize the complete public profile design, content, colors and visible sections.</p></div></div><div><button onClick={resetDesign}>↻ Reset Design</button><button className="is-primary" onClick={saveDesign}>✓ Save & Publish</button></div></header>
      <section className="vn-profile-provider-bar"><label>Choose Service Provider<select value={profileProviderId} onChange={(event)=>setProfileProviderId(event.target.value)}>{providerRows.map((item)=><option value={item.id} key={item.id}>{item.name} · {item.category}</option>)}</select></label><div><span className={`vn-admin-status-badge is-${String(provider.status).toLowerCase()}`}>{provider.status}</span><small>{provider.id} · {provider.city} · ★ {provider.rating}</small></div></section>
      <section className="vn-profile-manager-layout"><aside className="vn-profile-controls">
        <section><header><i>1</i><div><h3>Layout & Style</h3><p>Choose profile structure and shape.</p></div></header><label>Profile Template<select value={design.template} onChange={(event)=>updateDesign({template:event.target.value})}><option>Modern</option><option>Professional</option><option>Minimal</option><option>Bold</option></select></label><label>Card Corner Radius <strong>{design.cardRadius}px</strong><input type="range" min="0" max="28" value={design.cardRadius} onChange={(event)=>updateDesign({cardRadius:Number(event.target.value)})}/></label></section>
        <section><header><i>2</i><div><h3>Brand Colors</h3><p>Change the complete profile palette.</p></div></header><div className="vn-profile-color-grid">{[['primaryColor','Primary'],['accentColor','Accent'],['backgroundColor','Background'],['textColor','Text']].map(([key,label])=><label key={key}>{label}<span><input type="color" value={design[key]} onChange={(event)=>updateDesign({[key]:event.target.value})}/><input value={design[key]} onChange={(event)=>updateDesign({[key]:event.target.value})}/></span></label>)}</div></section>
        <section><header><i>3</i><div><h3>Profile Content</h3><p>Edit public-facing provider information.</p></div></header><label>Business Name<input value={design.businessName} onChange={(event)=>updateDesign({businessName:event.target.value})}/></label><label>Tagline<input value={design.tagline} onChange={(event)=>updateDesign({tagline:event.target.value})}/></label><label>About Provider<textarea rows="4" value={design.bio} onChange={(event)=>updateDesign({bio:event.target.value})}/></label><label>CTA Button Text<input value={design.ctaLabel} onChange={(event)=>updateDesign({ctaLabel:event.target.value})}/></label></section>
        <section><header><i>4</i><div><h3>Images</h3><p>Set cover and profile image paths.</p></div></header><label>Cover Image URL<input value={design.coverImage} onChange={(event)=>updateDesign({coverImage:event.target.value})}/></label><label>Avatar Image URL<input value={design.avatarImage} placeholder="Leave blank to show initials" onChange={(event)=>updateDesign({avatarImage:event.target.value})}/></label></section>
        <section><header><i>5</i><div><h3>Visible Sections</h3><p>Control what customers can see.</p></div></header><div className="vn-profile-switch-list">{[['showAbout','About Provider'],['showServices','Services'],['showPortfolio','Portfolio'],['showReviews','Reviews & Ratings'],['showContact','Contact Information']].map(([key,label])=><label key={key}><span>{label}</span><input type="checkbox" checked={design[key]} onChange={(event)=>updateDesign({[key]:event.target.checked})}/></label>)}</div></section>
      </aside><main className="vn-profile-preview-shell"><header><div><strong>Live Customer Preview</strong><small>{design.template} template · updates instantly</small></div><span>Desktop View</span></header><div className={`vn-profile-preview is-${design.template.toLowerCase()}`} style={{'--preview-primary':design.primaryColor,'--preview-accent':design.accentColor,'--preview-bg':design.backgroundColor,'--preview-text':design.textColor,'--preview-radius':`${design.cardRadius}px`}}>
        <div className="vn-profile-preview-cover" style={{backgroundImage:`linear-gradient(90deg, ${design.primaryColor}dd, ${design.primaryColor}55), url(${design.coverImage})`}}/><section className="vn-profile-preview-identity">{design.avatarImage?<img src={design.avatarImage} alt=""/>:<i>{initials}</i>}<div><span>✓ Verified Provider</span><h2>{design.businessName || provider.name}</h2><p>{design.tagline}</p><small>⌖ {provider.city}　·　{provider.category}</small></div><button>{design.ctaLabel || 'Request a Quote'}</button></section>
        <div className="vn-profile-preview-stats">{[[provider.rating,'Rating'],[provider.requests,'Requests'],[provider.portfolio,'Projects'],[provider.earnings,'Earnings']].map(([value,label])=><div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div>
        <section className="vn-profile-preview-about"><h3>About Us</h3><p>{design.bio}</p></section>
        {design.showServices?<section><div className="vn-profile-preview-heading"><h3>Our Services</h3><span>View all</span></div><div className="vn-profile-preview-cards">{(providerServices.length?providerServices:[{title:provider.category,price:'Custom pricing'},{title:'Professional Consultation',price:'Contact provider'}]).map((service,index)=><article key={service.id||index}><i><AdminIcon type={index%2?'spark':'briefcase'}/></i><strong>{service.title}</strong><small>{service.price}</small></article>)}</div></section>:null}
        {design.showPortfolio?<section><div className="vn-profile-preview-heading"><h3>Featured Work</h3><span>View portfolio</span></div><div className="vn-profile-preview-work">{(providerPortfolio.length?providerPortfolio:[{title:'Recent client project',category:provider.category},{title:'Business growth solution',category:'Featured'}]).map((project,index)=><article key={project.id||index} style={{background:`linear-gradient(145deg,${design.primaryColor}22,${design.accentColor}44)`}}><span>{project.category}</span><strong>{project.title}</strong></article>)}</div></section>:null}
        {design.showReviews?<section className="vn-profile-preview-review"><span>★★★★★</span><p>“Professional service, clear communication and excellent results.”</p><small>Verified customer review</small></section>:null}
        {design.showContact?<footer><strong>Ready to work with {design.businessName || provider.name}?</strong><button>{design.ctaLabel || 'Request a Quote'}</button></footer>:null}
      </div></main></section>
    </div>
  }
  ...
  const renderCategoriesSection = () => {
    ...
    const allSelected = visibleCategories.length > 0 && visibleCategories.every((category)=>selectedCategoryIds.includes(category.id))
    ...
    return <div className="vn-categories-page">
      ...
      <section className="vn-category-layout"><main className="vn-category-table-card"><div className="vn-category-tabs">{[['All','All Categories'],['Enabled','Active'],['Disabled','Disabled']].map(([value,label])=><button key={value} className={categoryStatus===value?'is-active':''} onClick={()=>{setCategoryStatus(value);setVisibleCategoryCount(8)}}>{label} ({value==='All'?categoriesData.length:categoriesData.filter((item)=>item.status===value).length})</button>)}</div><div className="vn-category-toolbar"><label><AdminIcon type="search"/><input value={searchQuery} onChange={(event)=>{setSearchQuery(event.target.value);setVisibleCategoryCount(8)}} placeholder="Search categories..."/></label><select value={categoryStatus} onChange={(event)=>{setCategoryStatus(event.target.value);setVisibleCategoryCount(8)}}><option value="All">All Status</option><option>Enabled</option><option>Disabled</option></select><select><option>Display Order</option><option>Most Services</option><option>Fewest Services</option></select><button onClick={()=>{setSearchQuery('');setCategoryStatus('All');setVisibleCategoryCount(8);pushToast('Category filters reset.')}}>☷ Reset</button></div>
      <div className="vn-category-table-wrap"><table className="vn-category-table"><thead><tr><th><input type="checkbox" checked={allSelected} onChange={()=>setSelectedCategoryIds(allSelected?[]:visibleCategories.map((item)=>item.id))}/></th><th>Category</th><th>Icon & Banner</th><th>Services</th><th>Subcategories</th><th>Display Order</th><th>Status</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{visibleCategories.map((category,index)=><tr key={category.id}><td><input type="checkbox" checked={selectedCategoryIds.includes(category.id)} onChange={()=>setSelectedCategoryIds((current)=>current.includes(category.id)?current.filter((id)=>id!==category.id):[...current,category.id])}/></td><td><div className="vn-category-name"><i><AdminIcon type={['grid','briefcase','chart','home','spark'][index%5]}/></i><div><strong>{category.name}</strong><small>{category.id}</small></div></div></td><td><span className="vn-category-asset">◫ {category.icon}</span><small>{category.image}</small></td><td><strong>{category.services}</strong></td><td>{category.subcategories}</td><td><span className="vn-category-order">{category.order}</span></td><td><StatusBadge value={category.status}/></td><td>Today, 10:{20+index} AM</td><td><div className="vn-category-actions"><button onClick={()=>setCategoryDialog({type:'view',category})} aria-label={`View ${category.name}`}>◉</button><button onClick={()=>setCategoryDialog({type:'edit',category})} aria-label={`Edit ${category.name}`}>✎</button><button onClick={()=>setCategoryDialog({type:'actions',category})} aria-label={`Actions for ${category.name}`}>⋮</button></div></td></tr>)}</tbody></table></div><footer className="vn-category-footer"><span>Showing {visibleCategories.length} of {filteredCategories.length} categories</span>{visibleCategories.length < filteredCategories.length?<button className="vn-category-show-more" onClick={()=>setVisibleCategoryCount((count)=>count+8)}>Show More</button>:null}</footer></main>
      ...
      {categoryDialog?<div className="vn-category-dialog-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&setCategoryDialog(null)}><div className={`vn-category-dialog${['add','edit'].includes(categoryDialog.type)?' is-detailed':''}`}><header><div><h3>{categoryDialog.type==='add'?'Add New Category':categoryDialog.type==='edit'?'Edit Category':categoryDialog.type==='view'?'Category Details':'Category Actions'}</h3><p>{categoryDialog.category?categoryDialog.category.name:'Set up how this category appears across the marketplace.'}</p></div><button onClick={()=>setCategoryDialog(null)} aria-label="Close">×</button></header>{['add','edit'].includes(categoryDialog.type)?<form onSubmit={(event)=>{event.preventDefault();const form=new FormData(event.currentTarget);const name=form.get('name').trim();const categoryDetails={name,slug:form.get('slug').trim()||name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),description:form.get('description').trim(),parent:form.get('parent'),icon:form.get('icon').trim()||'category.svg',image:form.get('image').trim()||'category-banner.png',status:form.get('status'),order:Number(form.get('order')),featured:form.get('featured')==='on',metaTitle:form.get('metaTitle').trim(),metaDescription:form.get('metaDescription').trim()};if(categoryDialog.type==='edit'){setCategoriesData((current)=>current.map((item)=>item.id===categoryDialog.category.id?{...item,...categoryDetails}:item));pushToast(`${name} updated.`)}else{setCategoriesData((current)=>[...current,{id:`C-${String(current.length+1).padStart(2,'0')}`,...categoryDetails,services:0,subcategories:0}]);pushToast(`${name} added.`)}setCategoryDialog(null)}}><div className="vn-category-form-body"><section><h4>Basic Information</h4><p>Give customers a clear name and short summary.</p><div className="vn-category-dialog-grid"><label>Category Name <span>*</span><input name="name" required placeholder="e.g. Website Development" defaultValue={categoryDialog.category?.name||''}/></label><label>URL Slug<input name="slug" placeholder="website-development" defaultValue={categoryDialog.category?.slug||''}/><small>Leave blank to generate automatically.</small></label><label className="is-wide">Description<textarea name="description" rows="3" placeholder="Describe the services customers can find in this category." defaultValue={categoryDialog.category?.description||''}/></label><label>Parent Category<select name="parent" defaultValue={categoryDialog.category?.parent||'None'}><option>None</option>{categoriesData.filter((item)=>item.id!==categoryDialog.category?.id).map((item)=><option key={item.id}>{item.name}</option>)}</select></label><label>Status<select name="status" defaultValue={categoryDialog.category?.status||'Enabled'}><option>Enabled</option><option>Disabled</option></select></label></div></section><section><h4>Media & Display</h4><p>Control the visual assets and listing position.</p><div className="vn-category-dialog-grid"><label>Icon Asset<input name="icon" placeholder="category.svg" defaultValue={categoryDialog.category?.icon||'category.svg'}/><small>SVG, PNG, GIF or asset path.</small></label><label>Banner Image<input name="image" placeholder="category-banner.png" defaultValue={categoryDialog.category?.image||'category-banner.png'}/><small>Recommended ratio: 16:9.</small></label><label>Display Order <span>*</span><input name="order" type="number" min="1" required defaultValue={categoryDialog.category?.order||categoriesData.length+1}/></label><label className="vn-category-check"><input name="featured" type="checkbox" defaultChecked={categoryDialog.category?.featured}/><span><strong>Featured category</strong><small>Highlight this category in discovery sections.</small></span></label></div></section><section><h4>Search Engine Preview</h4><p>Optional details used in browser and search results.</p><div className="vn-category-dialog-grid"><label className="is-wide">Meta Title<input name="metaTitle" maxLength="60" placeholder="Category name | VyaparNest" defaultValue={categoryDialog.category?.metaTitle||''}/></label><label className="is-wide">Meta Description<textarea name="metaDescription" rows="2" maxLength="160" placeholder="A concise description for search results." defaultValue={categoryDialog.category?.metaDescription||''}/></label></div></section></div><footer><small>Fields marked * are required.</small><button type="button" onClick={()=>setCategoryDialog(null)}>Cancel</button><button className="is-primary">{categoryDialog.type==='edit'?'Save Changes':'Create Category'}</button></footer></form>:null}{categoryDialog.type==='view'?<div className="vn-category-profile"><i><AdminIcon type="grid"/></i><h4>{categoryDialog.category.name}</h4><StatusBadge value={categoryDialog.category.status}/>{categoryDialog.category.description?<p>{categoryDialog.category.description}</p>:null}<dl><dt>Services</dt><dd>{categoryDialog.category.services}</dd><dt>Subcategories</dt><dd>{categoryDialog.category.subcategories}</dd><dt>Display Order</dt><dd>{categoryDialog.category.order}</dd><dt>Parent</dt><dd>{categoryDialog.category.parent||'None'}</dd><dt>Slug</dt><dd>{categoryDialog.category.slug||'—'}</dd><dt>Featured</dt><dd>{categoryDialog.category.featured?'Yes':'No'}</dd></dl><button onClick={()=>setCategoryDialog(null)}>Done</button></div>:null}{categoryDialog.type==='actions'?<div className="vn-category-action-menu"><button onClick={()=>setCategoryDialog({type:'view',category:categoryDialog.category})}>View category</button><button onClick={()=>setCategoryDialog({type:'edit',category:categoryDialog.category})}>Edit category</button><button onClick={()=>saveStatus(categoryDialog.category,categoryDialog.category.status==='Enabled'?'Disabled':'Enabled')}>{categoryDialog.category.status==='Enabled'?'Disable':'Enable'} category</button><button className="is-danger" onClick={()=>removeCategory(categoryDialog.category)}>Delete category</button></div>:null}</div></div>:null}
    </div>
  }
  ...
  const renderSectionContent = () => {
    ...
    if (activeSection === 'portfolio') {
      return renderProfileManagementSection()
    }

    if (activeSection === 'requests') {
      return renderServiceRequestsSection()
    }
